...
def window_version(os_version):
    """Takes the Windows Kernel version number and produces the associated consumer windows version."""
...
def os_distribution(data, start_date, end_date, country_list):
    """ Gets the distribution of OS usage calculated on the DAU, as well as a smoothed version (rolling 7 day mean). 
    ...
    dau_by_os_df = data.filter((col('submission_date_s3') > start_date) & (col('submission_date_s3') < end_date))\
        .groupBy(nice_os(col('os'), col('os_version')).alias('nice_os'), 'submission_date_s3', 'country')\
        .agg(countDistinct('client_id').alias('active_users'))\
        .cache()

    dau = dau_by_os_df.groupby('submission_date_s3', 'country')\
            .agg(sum('active_users').alias('dau'))

    dau_by_os_df = dau_by_os_df.join(dau, on = ['submission_date_s3', 'country'])\
        .select('*', (col('active_users') / col('dau')).alias('rate'))\
        .select('*', mean('rate').over(Window.partitionBy('nice_os', 'country')
                                             .orderBy('submission_date_s3')
                                             .rowsBetween(-6,0))
                                 .alias('smoothed_rate'))\
        .orderBy('nice_os', 'submission_date_s3')
    
    return dau_by_os_df.select('submission_date_s3', 'country', 'nice_os', 'rate', 'smoothed_rate')