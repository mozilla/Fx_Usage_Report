...
def getAvgIntensity(data, date, period = 7, country_list = None, locale_list = None):
    ...
    data1 = data.filter("submission_date_s3 <= '%s' and submission_date_s3 > '%s'"%(date, periodstartdate_str))\
                    .filter("subsession_length <= 86400")\
                     ...
                     .select('*', (col('total_daily_ticks')*5/col('total_daily_time')).alias('avg_daily_intensity'))
                    
    worldAvgIntensity = data1.groupBy('client_id')\
...
def getPeriodAvgIntensity(data, start_date, end_date, country_list, locale_list = None):
    ...
    for date in epoch_times:
        ...
        current_date = getAvgIntensity(data, date_str, period=7, country_list = country_list, locale_list = None)  
