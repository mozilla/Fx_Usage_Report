...
def getDailyAvgSession(data, date, period = 7, country_list = None, locale_list = None):
    """ Calculate Average Daily usage of the last 7 days for a particular date
        ...
        Returns: 
        a dataframe with four columns - 'submission_date_s3', 'country', 'avg_daily_subsession_length',
    ...
    data1 = data.filter("submission_date_s3 <= '%s' and submission_date_s3 > '%s'"%(date, periodstartdate_str))\
    ...
    if country_list is not None:   
        ...
        df = worldAvgSession.union(countryAvgSession).orderBy('submission_date_s3', 'country')    
...
def getAvgSession(data, start_date, end_date, country_list, locale_list = None):
    """ Calculate Average Daily usage for a period of dates
        ...
        a dataframe showing all the information for each date in the period
          - four columns: 'submission_date_s3', 'country', 'avg_daily_subsession_length','avg_daily_usage(hours)'
    """
    res = []
    epoch_times = pd.date_range(start_date, end_date, freq='D').astype('int64') / (10 ** 9)
    for date in epoch_times:
        date_str = time.strftime('%Y%m%d', time.localtime(date))
        current_date = getDailyAvgSession(data, date_str, period=7, country_list = country_list, locale_list = None)  
